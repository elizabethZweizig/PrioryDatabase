...
try
{
  if (isset($_GET['res_id'])) {
    ...
    $checkExistsInTable = $db->prepare("select count(*) from bedRes where bedResID = (:primary_key)");
    $exists = $checkExistsInTable->execute();

    if($exists > 0 && $primary_key > 0 ){
      $deleteStatement = $db->prepare("delete from bedRes where bedResID = (:primary_key);");
